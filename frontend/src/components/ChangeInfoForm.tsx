...
function ChangeInfoForm(props: {
  info: ProfileInfo | undefined;
  updateInfo: Dispatch<ProfileInfo>;
}) {
  ...
  const handleSubmit = async () => {
    ...
    await fetchdataAuth(
      "http://localhost:8080/changeInfo",
      "POST",
      undefined,
      info
    ).then((data: boolean) => {
  ...
  return (
    <div className='flex flex-col overflow-auto'>
      ...
      <div className='flex mt-5 ml-20 text-center'>
        ...
        <input
          className='container max-w-xs h-[2rem]  flex flex-col justify-center border border-black rounded-lg ml-[18.9rem]  pl-2 placeholder:text-black'
          type='password'
      ...
      <div className='flex justify-center w-full mt-10 '>
        <button
          ...
          className='px-6 py-2 text-white duration-150 ease-in bg-orange-500 rounded hover:bg-orange-700'
        >
          Submit
        </button>
      </div>
